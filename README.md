...
- The flyout's gear button switches to a manage view where you can toggle which layouts are active (persisted to `~/.local/state/omarchy/plugins/jordan.input-layout/config.json`).
...
```sh
omarchy plugin add https://github.com/jwkicklighter/jordan.input-layout.git --enable
```

It is placed in the center bar section by default. Move it with:

```sh
omarchy bar move jordan.input-layout --section right
```
...
Enabled layouts are stored in `~/.local/state/omarchy/plugins/jordan.input-layout/config.json`:
...
```sh
omarchy bar move jordan.input-layout --section center
```

## Remove

```sh
omarchy plugin remove jordan.input-layout
```

This removes the plugin code but leaves persisted state in `~/.local/state/omarchy/plugins/jordan.input-layout/`. Delete that directory to fully reset.
